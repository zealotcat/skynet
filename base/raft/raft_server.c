                                   ...
                                   msg_requestvote_response_t* r)
{
    ...
    __log(me_, node, "node `%d` responded to requestvote status: %s",
        raft_node_get_id(node),
        r->vote_granted == 1 ? "granted" : 
        r->vote_granted == 0 ? "not granted" : "unknown");
    ...
    __log(me_, node, "node `%d` responded to requestvote status:%s ct:%d rt:%d",
        raft_node_get_id(node),
        r->vote_granted == 1 ? "granted" :
        r->vote_granted == 0 ? "not granted" : "unknown",
        me->current_term,
...
int raft_send_appendentries(raft_server_t* me_, raft_node_t* node)
{
    ...
    __log(me_, node, "sending appendentries node `%d`: ci:%d comi:%d t:%d lc:%d pli:%d plt:%d",
        raft_node_get_id(node),
        raft_get_current_idx(me_),
